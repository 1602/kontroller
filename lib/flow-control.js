...
FlowControl.prototype.next = function (param) {
    var fn = this.context.localNext || this.context.next;
    fn(param);
};
 ...
 */
FlowControl.prototype.perform = function perform(actionName) {
    ...
    function getCaller(fn) {
        return function () {
            ctl.context.localNext = fn.isAction ? null : function (err) {
                if (err) {
                    ctl.context.next(err);
                } else {
                    run();
                }
            };
 ...
 */
Controller.prototype.skipBefore = function (name, only) {
 ...
 * @param {Array} or {String} only - choose actions to skip hook
 */
Controller.prototype.skipAfter = function (name, only) {
