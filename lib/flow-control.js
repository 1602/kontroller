...
FlowControl.prototype.call = function call(actionName) {
    ...
    if (f = this.compiledAction[actionName]) {
        console.log('found compiled action for', actionName);
        return f();
    }

    var action = this.constructor.actions[actionName];
    if (!action) {
        return this.next(new Error('Undefined action ' + this.controllerName + '#' + actionName));
    }
    ...
    this.compiledAction[actionName] = function () {
        ctl.context.applyAction(actionName, run);
        index = 0;
        run();
    };
