...
FlowControl.prototype.call = function call(actionName) {
    if (this.constructor.compiledAction[actionName]) {
        return this.constructor.compiledAction[actionName].call();
    }
    ...
    this.constructor.compiledAction[actionName] = function () {
        index = 0;
        run();
    };
    ...
    function getCaller(fn) {
        return function () {
            // console.log('[%d/%d] %s', index, len, fn.customName);
